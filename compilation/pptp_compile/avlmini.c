...
static inline void _avl_child_replace (struct avl_node *oldnode,
	struct avl_node *newnode, struct avl_node *parent, struct avl_root *root)
{
	if (parent)
	{
...
static inline struct avl_node *_avl_node_rotate_left (struct avl_node *node,
...
static inline struct avl_node *_avl_node_rotate_right (struct avl_node
	*node, struct avl_root *root)
{
...
} static inline struct avl_node *_avl_node_fix_l (struct avl_node *node,
	struct avl_root *root)
{
	struct avl_node *right = node->right;
	int rh0, rh1;
...
static inline struct avl_node *_avl_node_fix_r (struct avl_node *node,
	struct avl_root *root)
{
	struct avl_node *left = node->left;
	int rh0, rh1;
...
static inline void _avl_node_rebalance (struct avl_node *node,
	struct avl_root *root)
{
	while (node)
	{
...
/*====================================================================*/
void avl_tree_init (struct avl_tree *tree, int (*compare) (const void *,
		const void *), size_t size, size_t offset)
{
	tree->root.node = NULL;
	tree->offset = offset;
	tree->size = size;
