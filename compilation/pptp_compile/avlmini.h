...
#define avl_node_find(root, what, compare_fn, res_node) do {\
		...
		while (__n) { \
			...
			if (__hr == -1) { (res_node) = __n; break; } \
			else if (__hr == 1) { __n = __n->left; } \
			else { __n = __n->right; } \
		} \
	}   while (0)


#define avl_node_add(root, newnode, duplicate_node, dir) do { \
		struct avl_node **__link = &((root)->node); \
		struct avl_node *__parent = NULL; \
		struct avl_node *__duplicate = NULL; \
		int __hr = 1; \
		if(!__link[0]) root->head = newnode; \
		else { \
			if(dir == 1 && COMPARE_ASC(newnode, root->head) == 1) root->head = newnode; \
			else if(dir == -1 && COMPARE_DESC(newnode, root->head) == 1) root->head = newnode; \
		} \
		while (__link[0]) { \
			__parent = __link[0]; \
			if(dir == 1) __hr = COMPARE_ASC(newnode, __parent); \
			if(dir == -1) __hr = COMPARE_DESC(newnode, __parent); \
			if (__hr == -1) { __duplicate = __parent; break; } \
			else if (__hr == 1) { __link = &(__parent->left); } \
