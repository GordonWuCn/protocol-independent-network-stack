...
typedef struct pure_ip_data_t eth_ip_data_t;
struct eth_ip_ip_hdr_t{
	unsigned hdr_len:4;
	unsigned version:4;
	uint8_t TOS;
	uint16_t total_len;
...
struct eth_ip_rubik_bitmap_t{
	unsigned eth_ip_ip_hdr: 1;
	unsigned blank: 7;
};
struct eth_ip_select_t {
	uint32_t src_src_addr;
	uint32_t dst_dst_addr;
};
struct eth_ip_transition_bitmap_t{
...
extern struct eth_eth_header_t* eth_eth_header;
extern struct eth_rubik_bitmap_t* eth_rubik_bitmap;
extern struct eth_transition_bitmap_t* eth_transition_bitmap;
#endif
